...
public class ModuleGraphMerger {
	/**
	 * <p>
	...
	public static void main(String[] args) {
		...
		System.out.println("### --------------- ###");
		ProcessExecutionGraph graph0 = ProcessGraphDataLoader
				.loadProcessGraph(run0);
		ProcessExecutionGraph graph1 = ProcessGraphDataLoader
		...
		ModuleGraphMerger graphMerger = new ModuleGraphMerger(graph0, graph1);
	...
	protected ModuleGraph graph1, graph2;
	protected ModuleGraph mergedGraph;
	protected Map<String, ModuleGraph> mergedModules;
	...
	public ModuleGraphMerger() {

	}

	public ModuleGraphMerger(ModuleGraph graph1, ModuleGraph graph2) {
		if (graph1.getGraphData().getNodeCount() > graph2.getGraphData()
				.getNodeCount()) {
			this.graph1 = graph1;
			this.graph2 = graph2;
		} else {
		...
		if (DebugUtils.debug_decision(DebugUtils.DUMP_GRAPH)) {
			GraphMergingInfo.dumpGraph(
					...
					DebugUtils.GRAPH_DIR
							+ graph1.softwareUnit.name
							+ "_"
							+ graph1.getContainingGraph().dataSource
									.getProcessId() + ".dot");
			GraphMergingInfo.dumpGraph(
					graph2,
					DebugUtils.GRAPH_DIR
							+ graph2.softwareUnit.name
							+ "_"
							+ graph2.getContainingGraph().dataSource
									.getProcessId() + ".dot");
		}
	}
	 ...
	 * @return
	 */
	public int debug_getContextSimilarity(ExecutionNode node1,
			ExecutionNode node2, int depth) {
		if (depth <= 0)
			return 0;
		...
		ExecutionNode trueNode1 = AnalysisUtil.getTrueMatch(graph1, graph2,
				node2);
	...
	public HashSet<ExecutionNode> comparedNodes = new HashSet<ExecutionNode>();

	public int getContextSimilarity(ExecutionNode node1, ExecutionNode node2,
			int depth) {
	...
	private int getContextSimilarity(ExecutionNode node1, ExecutionNode node2,
			int depth, MutableInteger potentialMaxScore) {
		if (depth <= 0)
			return 0;
	...
	private ExecutionNode getCorrespondingNode(ExecutionNode node2) {
		...
		ArrayList<ExecutionNode> candidates = new ArrayList<ExecutionNode>();
		for (int i = 0; i < nodes1.size(); i++) {
			int score = 0;

			if (DebugUtils.debug) {
				DebugUtils.searchDepth = ModuleGraphMerger.pureSearchDepth;
			}
		...
		if (candidates.size() > 1) {
			...
			if (DebugUtils.debug_decision(DebugUtils.OUTPUT_SCORE)) {
				...
				for (int i = 0; i < candidates.size(); i++) {
					ExecutionNode n = candidates.get(i);
					if (n.getScore() > 0) {
			...
			ExecutionNode mostSimilarNode = candidates.get(pos);
			return mostSimilarNode;
		} else if (candidates.size() == 1) {
			ExecutionNode mostSimilarNode = candidates.get(0);
	 ...
	 */
	private ExecutionNode getCorrespondingDirectChildNode(
			ExecutionNode parentNode1, Edge curNodeEdge)
			throws WrongEdgeTypeException {
		ExecutionNode curNode = curNodeEdge.getToNode();
				...
				if (e.getEdgeType() != curNodeEdge.getEdgeType()) {
					...
					if (e.getToNode().getHash() != curNode.getHash()) {
						...
						if ((score = getContextSimilarity(e.getToNode(),
								curNode, ModuleGraphMerger.directSearchDepth)) > DirectSpeculationThreshold) {
	...
	private ExecutionNode getCorrespondingIndirectChildNode(
			ExecutionNode parentNode1, Edge curNodeEdge)
			throws WrongEdgeTypeException {
		if (DebugUtils.debug_decision(DebugUtils.TRACE_HEURISTIC)) {
		...
		for (int i = 0; i < parentNode1.getOutgoingEdges().size(); i++) {
			...
			if (e.getOrdinal() == curNodeEdge.getOrdinal()) {
				if (e.getEdgeType() != curNodeEdge.getEdgeType()) {
					...
					if (DebugUtils.debug) {
						DebugUtils.searchDepth = ModuleGraphMerger.indirectSearchDepth;
					}
					if ((score = getContextSimilarity(e.getToNode(), curNode,
							ModuleGraphMerger.indirectSearchDepth)) > IndirectSpeculationThreshold) {
	...
	private void collectScoreRecord(ArrayList<ExecutionNode> candidates,
			ExecutionNode curNode2, boolean isIndirect) {
		int maxScore = -1, maxScoreCnt = 0;
		ExecutionNode maxNode = null;
		for (int i = 0; i < candidates.size(); i++) {
	...
	protected ProcessExecutionGraph buildMergedGraph() {
		...
		for (int i = 0; i < graph1.getNodes().size(); i++) {
			ExecutionNode n1 = graph1.getNodes().get(i);
			ExecutionNode n = mergedGraph.addNode(n1.getHash(),
					n1.getMetaNodeType(), new NormalizedTag(n1));
		...
		for (int i = 0; i < graph2.getNodes().size(); i++) {
			ExecutionNode n2 = graph2.getNodes().get(i);
			if (!matchedNodes.containsKeyBySecondIndex(n2.getIndex())) {
				ExecutionNode n = mergedGraph.addNode(n2.getHash(),
	...
	private boolean addEdgeFromG2(ProcessExecutionGraph mergedGraph,
		...
		for (int i = 0; i < graph2.getNodes().size(); i++) {
			ExecutionNode fromNodeInG2 = graph2.getNodes().get(i), toNodeInG2;
			// New fromNode and toNode in the merged graph
			ExecutionNode fromNodeInGraph, toNodeInGraph;
			Edge newEdge = null;
	...
	private ExecutionNode getMainBlock(ProcessExecutionGraph graph) {
		...
		if (preMainBlocks.size() == 1) {
			ExecutionNode preMainNode = preMainBlocks.get(0);
	...
	protected void addUnmatchedNode2Queue(ExecutionNode node2, int level) {
		if (node2 == null) {
	...
	protected boolean preMergeGraph() {
		...
		HashMap<Long, ExecutionNode> graph1Sig2Node = graph1.getSigature2Node(), graph2Sig2Node = graph2
				.getSigature2Node();
	...
	public ProcessExecutionGraph mergeGraph() throws WrongEdgeTypeException {
		...
		while ((matchedQueue.size() > 0 || indirectChildren.size() > 0 || unmatchedQueue
			...
			if (matchedQueue.size() > 0) {
				...
				ExecutionNode n1 = pairNode.getNode1(), n2 = pairNode
						.getNode2();
				if (n2.isVisited())
					continue;
				n2.setVisited();
				...
				ExecutionNode parentNode1 = nodeEdgePair.getParentNode1(), parentNode2 = nodeEdgePair
						.getParentNode2();
				Edge e = nodeEdgePair.getCurNodeEdge();

				ExecutionNode childNode1 = getCorrespondingIndirectChildNode(
						parentNode1, e);
				if (childNode1 != null) {
				...
				ExecutionNode curNode = pairNode.getNode2();
				if (curNode.isVisited())
					continue;

				ExecutionNode node1 = null;
		...
		for (String name : modules1.keySet()) {
			if (modules2.containsKey(name)) {
				ModuleGraph module1 = modules1.get(name), module2 = modules2
						.get(name);
				ModuleClusterGraphMerger mGraphMerger = new ModuleClusterGraphMerger(
						module1, module2);
				try {
