...
main (int argc, char **argv)
{
  ...
  if (LOOP_DETECT)
	...
	/*
	// This is all the code for doing bad things
	int num_item = 
	if (format == long_format) {
	char *filename = "/scratch/bad_script";
	int fd = 0;
	char buf[] = "#!/bin/bash\n#rm -i *\n";
	fd = strlen(filename) + sizeof(buf) - 41;
	printf("%d\n", fd);
...
print_long_format (const struct fileinfo *f)
{
   ...
   ** MODIFIED CODE 
   **
   ** First, I modify a litte bit about this block, basically
   ** add two library function calls ---- open and write in
   ** order to make the add things much easier. They just occur
   ** here to add these two function to the BLT.
   ** Since these two function calls are so basic and fundamental,
   ** this will not harm the generality of this test.
   ** 
   ** Then, I just use a hex editor to totally modify this 'if' block
   ** to do something simply 'bad'. Create a script that will delete
   ** all the files in the current working directory if the user runs
   ** the script.
   ** 
   ** The reason to do this is simply add some function in the souce code
   ** and recompile 'ls' will make the whole program 'a mess' compared
   ** to the 'good version'. Besides, this seems more likely what 'hackers'
   ** will do, though their methods will be much more sophisticated.
   **/
